...
function renderSchedule(day) {
    ...
    if (lessons.length === 0) {
        container.innerHTML = '<div class="text-center text-gray-500 dark:text-white/50 py-8">В этот день уроков нет 🙌</div>';
        return;
    }
    ...
    container.innerHTML = lessons.map((lesson, index) => {
        ...
        if (realToday === day) {
            if (currentTime >= startTime && currentTime < endTime) {
                isCurrent = true;
                statusBadge = '<span class="text-xs bg-green-400/20 text-green-700 dark:text-green-300 px-2 py-0.5 rounded-full ml-2">Идёт урок</span>';
            }
        }
        ...
        return `
            <div class="lesson-card glass-dark rounded-xl p-3 md:p-4 flex items-center gap-4 ${isCurrent ? 'current' : ''}">
                <div class="text-center min-w-[60px]">
                    <div class="text-gray-800 dark:text-white font-bold text-base md:text-lg leading-none mb-1">${lesson.time}</div>
                    <div class="text-gray-600 dark:text-white/50 text-xs">${lesson.endTime}</div>
                </div>
                <div class="h-10 w-px bg-gray-300 dark:bg-white/10"></div>
                <div class="flex-1 min-w-0">
                    <div class="flex items-center flex-wrap gap-1">
                        <span class="text-gray-800 dark:text-white font-semibold text-base md:text-lg truncate">${lesson.name}</span>
                        ${statusBadge}
                    </div>
                    <div class="flex items-center gap-2 text-gray-600 dark:text-white/60 text-sm mt-0.5">
...
function updateTimer() {
    ...
    if (state === 'lesson') {
        ...
        elState.className = 'text-sm text-blue-600 dark:text-blue-300 mt-2 bg-white/20 dark:bg-white/10 px-3 py-1 rounded-full';
        ...
        elState.className = 'text-sm text-green-600 dark:text-green-300 mt-2 bg-white/20 dark:bg-white/10 px-3 py-1 rounded-full';
